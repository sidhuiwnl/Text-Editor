...
document.addEventListener('keydown', (event) => {
    ...
    if(event.key === 'z' && (event.ctrlKey || event.metaKey)){
        ...
        const cmd = event.shiftKey ? history.redo(gb) : history.undo(gb);
        ...
        if (cursor > 0) {
            const deletedChar = gb.toString()[cursor - 1];
            history.push({ type: 'delete', pos: cursor - 1, char: deletedChar });
            gb.delete(cursor);
        ...
        gb.insert(cursor, '\n');
         history.push({ type: 'insert', pos: cursor, char: '\n' });
        cursor++;
        rememberedCol = 0;
